...
def main(set, test, mspacing,mratio, rhomin, rhomax, Lrange, delta, Sigma, 
    ...
    # ---模型
    # ---模型,实例化类
    model = potential.GravMagModule(dobs, mrange, mspacing, obsurface, mratio=mratio,
...
if __name__ == "__main__":
    ...
    mratio =  parameters[attempt]["mratio"]
    # dt & L
    ...
    # calculate
    start = time.time()
    main(set, test, mspacing, mratio, rhomin, rhomax, Lrange, delta, Sigma,
        RegulFactor, regularization, beta, nsamples)
    end = time.time()
