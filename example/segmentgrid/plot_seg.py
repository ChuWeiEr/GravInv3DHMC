...
for chain in range(0, chains):
    ...
    # mesh模块
    # ---------反演模型的均值
    mesh_inv_mean = mesher.PrismMeshSegment((xmin, xmax, ymin, ymax, zmin, zmax), 
                                            (dz, dy, dx), divisionsection)
    mesh_inv_mean.addprop('density', model_inv_mean)
    # ---------反演模型的方差
    mesh_inv_std = mesher.PrismMeshSegment((xmin, xmax, ymin, ymax, zmin, zmax), 
                                            (dz, dy, dx), divisionsection)
    ...
    plt.xticks(np.arange(0, ny + 1, 10))
    plt.yticks(np.arange(0, nz + 1, 5))
    plt.xlim([0, ny])
    plt.ylim([0, nz])
    plt.xlabel("Easting(m)")
    plt.ylabel("Depth(m)")
