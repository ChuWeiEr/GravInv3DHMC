...
gravmag.tesseroid as a backend and are not meant to be used directly.
            ...
            while stktop >= 0:
                ...
                if new_cells > 1:
                    if new_cells + (stktop + 1) > stack.shape[0]:
                        raise OverflowError
                    # stktop就是划分棱柱的个数，新划分的棱柱都存放在stack里面了
                    stktop = split(w, e, s, n, top, bottom, nlon, nlat, nr,
                                   stack, stktop)
                else:
                    # 当 当前这个小棱柱都不需要继续划分时，做计算
                    ...
                    result[l] += density*scale*kernel(
                        ...
                        coslatc, rc)
                    # 除了密度之外的所有项都是正演的kernel，tessnum是当前计算小棱柱的序号
                    kernel2d[l, tessnum] += scale*kernel(
                        lon[l], coslat[l], sinlat[l], radius[l], lonc, sinlatc,
                        coslatc, rc)
...
# that will be used by gravmag.tesseroid
